...
- [Backend] `listServiceBoards()` now excludes any board assigned to the "Admin" ConnectWise department (verified live: the "Admin" and "Triton Management" boards) — internal admin work on those boards shouldn't be selectable as a vessel-tracking priority signal. Same exclusion extended to the Project-activity signal (`queryOpenProjectActivity`, via each CW Project's own `board` field, `board/name not in (...)`) — Admin-board projects don't feed Tier 1 priority either.
