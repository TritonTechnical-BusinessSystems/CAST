...
- **Fleshing-out notes:**
  ...
  - **Done 2026-08-09:** the `POST` write-back was only gated on `requireAuth`, not `vessel.reconcile` as intended — any signed-in viewer could write, relying solely on the (now in-app-toggleable, `INIT-0013`) `CW_WRITES_ENABLED` gate. Fixed. Also added a **temporary** mass-entry page, `/vessel-identity-quick-entry` (linked from the Identity tab) — inline per-row fields + a "Save all" for clearing the backlog fast, built entirely on the existing routes above. Delete once the backlog clears (see the page file's own header comment).
