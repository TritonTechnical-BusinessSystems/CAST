...
> Status: Governance & knowledge canon established; both components have code. The **browser extension** has a working MV3 runtime (content-script rule engine, service-worker config-poll + check-in phone-home, popup) packed + signed into a self-hosted, self-updating `cast.crx` with a one-click force-install installer served from the app — not yet load-tested on a real managed device. Repack via `scripts/pack-extension.sh`. The **CAST web app** runs on Vite React SPA (`components/web`) + Express API (`components/api`) + shared `packages/config-schema`: a **download-first public landing (`/download`)** for grabbing the extension, AD-LDAPS-+-local-fallback config sign-in (`/login`) over a JWT cookie, route protection, and two tabbed Workspace sections — **Browser Extension** (Role Rules/Expected Pods placeholders, **Fleet live** — per-member check-in catalog with deployment-status filter, Deployment) and **Vessel Tracking** (Vessel Location/Identity/Tracking Config/Geo Alerts). All UI passes through the `ux-designer` review agent before deploy. Typechecks/builds/runtime-verified; deploys via Docker to `trt-cast-01` at `cast.tritontechnical.com` (`knowledge/architecture/cast-web-app-vm-provisioning.md`). No release tagged yet — first build will be `v0.1.0.0` when the user declares it.
